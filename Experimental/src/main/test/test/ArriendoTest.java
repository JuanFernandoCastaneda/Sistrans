...
public class ArriendoTest
{
	 ...
	 */
	private static Logger log = Logger.getLogger(ArriendoTest.class.getName());
	...
	public void unicidadUsuarioTest() 
	{
		...
		try
		{
			log.info ("Probando la restricción de UNICIDAD de Arriendo");
			alohAndes = new PrincipalAlohAndes (openConfig (CONFIG_TABLAS_A));
		}
		catch (Exception e)
		{
//			e.printStackTrace();
			log.info ("Prueba de UNICIDAD de Arriendo incompleta. No se pudo conectar a la base de datos !!. La excepción generada es: " + e.getClass ().getName ());
			log.info ("La causa es: " + e.getCause ().toString ());

			String msg = "Prueba de UNICIDAD de Usuario incompleta. No se pudo conectar a la base de datos !!.\n";
			msg += "Revise el log de y el de datanucleus para conocer el detalle de la excepción";
		...
		try
		{
			...
			//
			VOArriendo arriendo1 = alohAndes.adicionarReserva(1, 1, 2, null, 1);

			VOArriendo arriendo2 = alohAndes.adicionarReserva(1, 1, 2, null, 1);
			assertNull ("No puede adicionar dos usuarios con el mismo id !!", arriendo2);
		}
		catch (Exception e)
		{
//			e.printStackTrace();
